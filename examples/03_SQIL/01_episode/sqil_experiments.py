...
from imitation_lib.imitation import SQIL
...
def _create_agent(mdp, expert_data, sw, lr_critic, lr_actor, plcy_loss_mode,
                  use_target, use_cuda, tau,
                  learnable_alpha, init_alpha, reg_mult, gamma,
                  log_std_min, log_std_max, delay_Q, n_fits,
                  logging_iter):
    ...
    # create SQIL agent
    agent = SQIL(mdp_info=mdp.info, batch_size=batch_size, initial_replay_size=initial_replay_size,
                 ...
                 learnable_alpha=learnable_alpha, init_alpha=init_alpha,
                 reg_mult=reg_mult, log_std_min=log_std_min, log_std_max=log_std_max,
                 n_fits=n_fits, logging_iter=logging_iter)
...
def experiment(env_id: str = "HalfCheetah-v2",
    ...
    agent = _create_agent(mdp, expert_data,  sw=tb_writer, lr_critic=lr_critic, lr_actor=lr_actor,
                          plcy_loss_mode=plcy_loss_mode,
                          use_cuda=use_cuda, use_target=use_target,
                          delay_Q=delay_Q, tau=tau, learnable_alpha=learnable_alpha, init_alpha=init_alpha,
                          reg_mult=reg_mult, gamma=gamma,
